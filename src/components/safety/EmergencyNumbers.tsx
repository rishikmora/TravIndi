...
const NUMBERS = [
  ...
  { key: 'touristHelpline', number: '1363' },
] as const;
    ...
    <section aria-labelledby="emergency-numbers-title" className={cn('grid gap-3 rounded-3xl bg-[var(--tone-danger-bg)] p-4 md:p-5', className)}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {NUMBERS.map((entry) => (
          ...
          >
            {t(`safety.emergency.${entry.key}`)} <span className="tabular-nums">{entry.number}</span>
          </a>
        ))}
      </div>
      {!compact && <p className="text-[0.875rem] text-[var(--text-muted)]">{t('safety.emergency.notConnected')}</p>}
    </section>
  );
}
