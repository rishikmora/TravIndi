...
export function createTripRepository(client: RepositoryClient): TripRepository {
  return {
    ...
    create: (input, options) => client.post<TripDto>(endpoints.trips.create, input, options),
    update: (tripId, patch) => client.patch<TripDto>(endpoints.trips.detail(tripId), patch),
    extractIntent: (input, options) => client.post<IntentExtractionDto>(endpoints.trips.extractIntent, input, options),
    generateItinerary: (tripId, options) => client.post<GenerationJobDto>(endpoints.trips.generate(tripId), {}, options),
