...
| C2 | RAG orchestration library (LlamaIndex vs. LangChain — never resolved) | **LlamaIndex** | Narrower, more retrieval-focused API surface; fewer moving parts to configure correctly for a grounded tourism-knowledge RAG use case than LangChain's broader agent framework | Phase 12 (AI/RAG) |
