    ...
    if (showPeople) {
      for (const share of shares.data ?? []) {
        if (share.lastLocation && memberShareIds.has(share.shareId)) {
          list.push({ id: share.shareId, lat: share.lastLocation.latitude, lng: share.lastLocation.longitude, label: share.ownerName, kind: 'person', detail: now ? locationFreshnessLabel(locationFreshness(share.lastLocationAt, now)) : null });
        }
      }
    }
    const connector: MapLine[] = stops.length > 1 ? [{ id: 'order', coordinates: stops.map((s) => [s.place!.coordinates!.lng, s.place!.coordinates!.lat]), label: t('itinerary.map.orderLine'), tone: 'connector' }] : [];
    return { points: list, lines: connector };
  }, [activeDay, showPeople, shares.data, trip.data, now, t]);
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <SegmentedControl
          label={t('itinerary.map.dayLabel')}
          size="sm"
          value={String(activeDay?.dayNumber ?? '')}
          onChange={(value) => setDayNumber(value)}
          options={itinerary.data.days.map((d) => ({ value: String(d.dayNumber), label: t('itinerary.view.day', { day: d.dayNumber }) }))}
        />
        <div className="flex flex-wrap gap-2">
          <ToggleChip selected={showPeople} onToggle={() => setShowPeople((v) => !v)}>
            {t('itinerary.map.showPeople')}
          </ToggleChip>
          <ButtonLink href={`/routes?trip=${tripId}`} variant="secondary" size="sm">
            {t('itinerary.map.compareRoutes')}
          </ButtonLink>
        </div>
      </div>
      <InlineNotice tone="neutral">{t('itinerary.map.dashedNote')}</InlineNotice>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        <MapView points={points} lines={lines} selectedId={highlighted} onSelect={setSelected} label={t('itinerary.map.dayStops', { day: activeDay?.dayNumber ?? '' })} className="aspect-[4/3] w-full" />
        <ol className="surface-card grid content-start gap-1 p-3" aria-label={t('itinerary.map.dayStopsInOrder', { day: activeDay?.dayNumber ?? '' })}>
          {points.map((point) => (
            <li key={point.id}>
          ...
          {points.length === 0 && <li className="p-2 text-[var(--text-muted)]">{t('itinerary.map.noStops')}</li>}
        </ol>
      </div>
    </div>
  );
